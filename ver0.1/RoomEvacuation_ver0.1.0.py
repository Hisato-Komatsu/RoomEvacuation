...
class hyperparameters:
   def __init__(self):
      ...
      self.gamma_intr = 0.9
      self.coef_intr = 0.05
      self.decay_ratio_intr = 0.8 #decay of memory matrices per one update of W_out_intr
...
class main_window(Env):
   def __init__(self, hyperparameters):
      ...
      self.gamma_intr_label = tk.Label(self.label_frame, text="gamma(intrinsic)", width=self.label_width)
      ...
      self.gamma_intr_txtbox = tk.Entry(self.label_frame, width=self.txtbox_width)
      ...
      self.gamma_intr_txtbox.insert(tk.END, self.gamma_intr)
      ...
      self.gamma_intr_label.grid(column=0, row=3)
      ...
      self.gamma_intr_txtbox.grid(column=1, row=3)
      self.label_frame.grid(column=self.cols+1, row=0, columnspan=3, rowspan=5, padx=2, pady=2)

      self.seed_frame = tk.LabelFrame(self.frame,text="")
      self.random_seed_label = tk.Label(self.seed_frame, text="random seed", width=self.label_width)
      self.random_seed_txtbox = tk.Entry(self.seed_frame, width=self.txtbox_width)
      self.random_seed_txtbox.insert(tk.END, self.seed)

      self.random_seed_label.grid(column=0, row=0)
      self.random_seed_txtbox.grid(column=1, row=0)
      self.seed_frame.grid(column=self.cols+1, row=5, columnspan=3, rowspan=2, padx=2, pady=2)

      self.start_btn = tk.Button(self.frame, text='Start', width=10, command=self.start_simulation)
      ...
      self.conf1_btn.grid(column=self.cols+1,row=self.rows-7, columnspan=1, rowspan=3, padx=2, pady=2)
      self.conf2_btn = tk.Button(self.frame, text='conf2', width=10, command=self.load_conf2)
      self.conf2_btn.grid(column=self.cols+2,row=self.rows-7, columnspan=1, rowspan=3, padx=2, pady=2)
      self.conf3_btn = tk.Button(self.frame, text='conf3', width=10, command=self.load_conf3)
      self.conf3_btn.grid(column=self.cols+3,row=self.rows-7, columnspan=1, rowspan=3, padx=2, pady=2)

      self.save_conf_btn = tk.Button(self.frame, text='save conf', width=10, command=self.save_conf)
      self.save_conf_btn.grid(column=self.cols+1,row=self.rows-6, columnspan=2, rowspan=3, padx=2, pady=2)
      self.load_conf_btn = tk.Button(self.frame, text='load conf', width=10, command=self.load_saved_conf)
      self.load_conf_btn.grid(column=self.cols+2,row=self.rows-6, columnspan=2, rowspan=3, padx=2, pady=2)
   ...
   def start_simulation(self):
      if self.global_n_episode == 0:
         ...
         g_intr_temp = float( self.gamma_intr_txtbox.get() )
         if self.check_0to1([g_temp,ed_temp,em_temp,g_intr_temp]):
            self.gamma = g_temp
            self.epsilon_decay = ed_temp
            self.epsilon_min = em_temp
            self.gamma_intr = g_intr_temp
         else:
            raise ValueError("Invalid parameter is given.")
         seed_temp = int(self.random_seed_txtbox.get())
         if seed_temp < 0:
         ...
         self.gamma_txtbox["state"] = tk.DISABLED
         self.epsilon_decay_txtbox["state"] = tk.DISABLED
         self.epsilon_min_txtbox["state"] = tk.DISABLED
         self.gamma_intr_txtbox["state"] = tk.DISABLED
         self.random_seed_txtbox["state"] = tk.DISABLED
         self.conf1_btn["state"] = tk.DISABLED
         self.conf2_btn["state"] = tk.DISABLED
         self.conf3_btn["state"] = tk.DISABLED
         self.save_conf_btn["state"] = tk.DISABLED
         self.load_conf_btn["state"] = tk.DISABLED
   ...
   def run(self):
      ...
      for n_episode in range (self.ep_termination):
         self.reset()
         if (self.global_n_episode+1) in {1,10,20} or (n_episode+1) == self.ep_termination : 
   ...
   def reset_whole(self):
      ...
      self.gamma_txtbox["state"] = tk.NORMAL
      self.epsilon_decay_txtbox["state"] = tk.NORMAL
      self.epsilon_min_txtbox["state"] = tk.NORMAL
      self.gamma_intr_txtbox["state"] = tk.NORMAL
      self.random_seed_txtbox["state"] = tk.NORMAL
      self.conf1_btn["state"] = tk.NORMAL
      self.conf2_btn["state"] = tk.NORMAL
      self.conf3_btn["state"] = tk.NORMAL
      self.save_conf_btn["state"] = tk.NORMAL
      self.load_conf_btn["state"] = tk.NORMAL
      self.file_LC.close()
      self.file_LC = open(self.filename_LC, "w")
   ...
   def save_conf(self) :
      state_temp = np.zeros((self.height,self.width,3)).astype(int)
      for i_col in range(self.width):
         for i_row in range(self.height):
            state_temp[i_row][i_col] = self.btn[i_row][i_col].color_state
      np.save('saved_conf', state_temp)

   def load_saved_conf(self) :
      state_temp = np.load('saved_conf.npy')
      for i_col in range(3,self.width-3):
         for i_row in range(3,self.height-3):
            if np.array_equal( state_temp[i_row][i_col], np.array([1,0,0]) ) :
               self.btn[i_row][i_col].button["bg"] = "red"
            elif np.array_equal( state_temp[i_row][i_col], np.array([0,1,0]) ) :
               self.btn[i_row][i_col].button["bg"] = "green"
            elif np.array_equal( state_temp[i_row][i_col], np.array([0,0,1]) ) :
               self.btn[i_row][i_col].button["bg"] = "blue"
            elif np.array_equal( state_temp[i_row][i_col], np.array([0,0,0]) ) :
               self.btn[i_row][i_col].button["bg"] = "black"
            else:
               raise ValueError('Invalid color value!')
            self.btn[i_row][i_col].color_state = state_temp[i_row][i_col]
